...
- feat: |Admin| 新增角色管理页面，支持在后台新增和编辑角色，并将角色定义持久化到系统设置中
- feat: |登录| 新增 `REQUIRE_USER_LOGIN` 模式，未登录用户账号时不再允许进入系统、创建邮箱或直接使用邮箱密码登录；首页改为统一用户登录入口，邮箱地址改为登录后创建/绑定

### Bug Fixes

- fix: |登录| 邮箱登录默认切换为“邮箱 + 密码”流程，保留 JWT 作为底层会话令牌，不再把粘贴 credential/JWT 作为前台默认登录方式
- fix: |地址密码| 统一地址密码修改、登录校验与后台重置的存储规则，兼容前端传入哈希值并避免密码比较逻辑不一致

### Improvements

- style: |前端| 调整首页邮箱头部账户弹层，展示当前邮箱信息，并补充用户设置入口以适配新的工作区壳层
